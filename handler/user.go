...
func (h *userHandler) CheckEmailAvailability(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIResponse("Email checkhing failed", http.StatusUnprocessableEntity, "error", errorMessage)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
...
func (h *userHandler) UploadAvatar(c *gin.Context) {
	...
	// JWT (sementara hardcode untuk use login ID = 1)
	...
	// avatar file name format : Idnumber-filename.ex
...
func (h *userHandler) FetchUser(c *gin.Context) {
	...
	formater := user.FormatUser(currentUser, "")

	response := helper.APIResponse("Successfuly fetch user data", http.StatusOK, "success", formater)
